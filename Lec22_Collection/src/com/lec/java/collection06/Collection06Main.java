...
public class Collection06Main {
	...
	public static void main(String[] args) {
		...
		// 데이터 저장: add()
		// set.add 인덱스가 없기 때문에 
		// retunr값 boolean 성공하면(중복X : true), 실패하면(중복O : false) 
		set.add(100);	
		set.add(200);
		set.add(300);
		set.add(100);
...
//		System.out.println(set1);
		...
		System.out.println("삭제 결과 " + set.remove(2));
		//	set.remove(2);	// 데이터 2를 삭제, (2번째 삭제가 아니다!!)
		
		
		System.out.println();
		System.out.println("삭제 후:");

		itr = set.iterator();	// 일회용이라 다시 넣어줘야한다
		while(itr.hasNext()) {
			System.out.println(itr.next());
			// 출력결과, 저장순서와 관계 없다.
		}
		...
		set.remove(600);
		set.add(777);
		// 500 -> 555 하고 싶으면 500 지우고 555추가하면된다.
