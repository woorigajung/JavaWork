...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		String input;
		String regex; // 정규표현식 : 정규표현식도 문자열 형식
		Pattern pat;
		Matcher matcher;
		...
		matcher = pat.matcher(input); // Matcher객체 생성하고 리턴
		...
		if (matcher.find()) {
			...
			System.out.println(matcher.group() + "{" + matcher.start() + "~" + matcher.end() + "}"); // 해보니까 '다음' 매칭이란?
																										// 매칭은 왼쪽부터 차례대로
																										// 실행된다 따라서
																										// 이문장을 다시 사용하면
																										// 다음 패턴을 찾는다
		...
		// reset() 다시 처음부터 패턴매칭하도록 reset 함.
		matcher = matcher.reset(); // 따라서 다시 매칭하고싶으면 매칭한 객체를 리셋한다
		...
		if(matcher.matches()) {
			...
			System.out.println("matcher() 매칭 FAIL");
		}
		...
		if(Pattern.matches("My....", "Myabc")) {	// 정규표현식은 기본적으로 대소문자를 가림 
		...
		System.out.println(matcher.replaceAll("***"));	//매칭된 부분만 변경
		...
		while(matcher.find(fromIndex)) {	//hasnext같은 동작
			System.out.println(matcher.group() + "{" + matcher.start() + "~" + matcher.end() + "}");
